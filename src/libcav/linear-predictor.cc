...
{
	uint uSize = (m_f.u().rows() * m_f.u().cols());
	...
	for(uint r = 0; r < m_f.u().rows(); r++) {
		for(uint c = 0; c < m_f.u().cols(); c++) {
			...
			if(c == 0 && r == 0) {
				// First pixel at F[0][0], all values are considered to be 0
				errors[m_f.size() + (c + r)] = x;
				errors[m_f.size() + uSize + (c + r)] = xv;
			} else if (r == 0) {
				// First row b and c are considered to be 0 (no previous value)
				a = m_f.u()[r * m_f.u().cols() + c - 1];
				av = m_f.v()[r * m_f.v().cols() + c - 1];
				
				errors[m_f.size() + (c + r)] = x - m_functor(a, 0, 0);
				errors[m_f.size() + uSize + (c + r)] = xv - m_functor(av, 0, 0);
			} else if(c == 0) {
				// First column a and b are considered to be 0
				// a and c  are zeros 

				b = m_f.u()[(r - 1) * m_f.u().cols() + c];
				errors[m_f.size() + (c + r)] = x - m_functor(0, b, 0);

				bv = m_f.v()[(r - 1) * m_f.v().cols() + c];
				errors[m_f.size() + uSize + (c + r)] = xv - m_functor(0, bv, 0);
			} else { 
				a = m_f.u()[r * m_f.u().cols() + c - 1];
				b = m_f.u()[(r - 1) * m_f.u().cols() + c];
				c2 = m_f.u()[(r - 1) * m_f.u().cols() + c - 1];
				errors[m_f.size() + (c + r)] = x - m_functor(a, b, c2);

				av = m_f.v()[r * m_f.v().cols() + c - 1];
				bv = m_f.v()[(r - 1) * m_f.v().cols() + c];
				cv = m_f.v()[(r - 1) * m_f.v().cols() + c - 1];
				errors[m_f.size() + uSize + (c + r)] = xv - m_functor(av, bv, cv);
			}
		}
	}
	...
	for(uint r = 0; r < f.rows(); r++) {
		for(uint c = 0; c < f.cols(); c++) {
			...
			if(c == 0 && r == 0) {
				...
				// b and c2 are zero 
				f.y()[r * f.cols() + c] = e + m_functor(a, 0, 0);
			} else if(c == 0) {
				// First column a and b are considered to be 0
				// a and c  are zeros 
	...
	for(uint r = 0; r < f.u().rows(); r++) {
		for(uint c = 0; c < f.u().cols(); c++) {
			...
			if(c == 0 && r == 0) {
				...
				f.u()[(c + r)] = e + m_functor(a, 0, 0);
				f.v()[(c + r)] = ev + m_functor(av, 0, 0);
			} else if(c == 0) {
				// First column a and b are considered to be 0
				// a and c  are zeros 

				b = f.u()[(r - 1) * f.u().cols() + c];
				f.u()[(c + r)] = e + m_functor(0, b, 0);

				bv = f.v()[(r - 1) * f.v().cols() + c];
				f.v()[(c + r)] = ev + m_functor(0, bv, 0);

			} else { 
				a = f.u()[r * f.u().cols() + c - 1];
				b = f.u()[(r - 1) * f.u().cols() + c];
				c2 = f.u()[(r - 1) * f.u().cols() + c - 1];
				f.u()[(c + r)] = e + m_functor(a, b, c2);

				av = f.v()[r * f.v().cols() + c - 1];
				bv = f.v()[(r - 1) * f.v().cols() + c];
				cv = f.v()[(r - 1) * f.v().cols() + c - 1];
				f.v()[(c + r)] = ev + m_functor(av, bv, cv);
			}
		}
	}
